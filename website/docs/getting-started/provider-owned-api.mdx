...
| `POST /capabilities/documents`，显式选择 `route.http` | `readDocuments({ selected: ['route.http'], requiredStaticRevision })` | 返回该能力知识的逐项内容/错误 |
...
数据库权威、能力召回、知识检索和远程 Reader 在 `1.0.0` 中提供公开合同，但不附带真实后端。只有 Provider 配置了对应 Adapter，相关原语才可用。
