...
成功输出必须包含 `catalog: ["route", "route.http"]`、`detail: "route.http"`、`parents: ["route"]` 和 `completeness: "complete"`。这证明静态定义、显式关系和本地知识已经通过同一公开入口跑通。下文逐项解释同一示例为什么这样建模，以及如何放进常见的 `providers/acme-http` 项目布局。
...
| `specification` | 否 | Provider 全局使用规则的元数据；不需要规范入口时可整体省略 |
...
| `entryRef` | 否 | 规范正文入口；本例按接入方约定从 Provider 目录读取 `PROVIDER.md` |
...
这里用 `parents: ["route"]` 表达 HTTP 路由属于路由能力族。`route.http` 中的点号只是命名约定，省略 `parents` 就没有父子关系。只有“保留另一能力核心语义的更具体变体”才使用 `specializes`；`related` 只提示显式关联。它们都可省略，不必为每个节点凑齐三类关系。

| Knowledge 字段 | 必填 | 含义 |
|---|---|---|
| `knowledge` | 否 | 本能力关联的知识引用；没有知识时可省略 |
| `kind` | 引用内是 | `document` 表示可精确读取的一篇文档；Collection 则需显式检索 |
...
这证明定义可加载、显式父关系可查询、本地知识可读取。`requiredStaticRevision` 来自本次 Catalog，后续查询沿用它，不能填写 Provider 的 `version`。知识选择必须显式非空，不会自动包含父节点或关联能力。
