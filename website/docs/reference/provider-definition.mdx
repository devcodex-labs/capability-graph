...
`specification` 可整体省略；提供时必须包含非空 `specificationId` 和 `version`。`appliesTo` 及其 `software`、`versionRange`、`conditions` 均可选，`entryRef` 也可选。Core 只关联这些元数据，不读取正文来执行策略，也不判断当前项目版本。
