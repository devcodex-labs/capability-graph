...
for (let i = 0; i < 5; i++) {
  ...
  assert.deepEqual(result.selection.added.map((item) => item.capabilityId), ["schema.request"]);
  assert.deepEqual(result.specification.results.flatMap((entry) => entry.ok ? [entry.value.knowledgeId] : []), ["SPEC-01"]);
  assert.equal(result.catalog.items.length, 5);
  const bytes = Object.fromEntries(["providers", "catalog", "detail", "neighbors", "selection", "documents", "specification"].map((name) =>
    [name, Buffer.byteLength(JSON.stringify(result[name]), "utf8")]));
  assert.ok(bytes.catalog <= 24576);
  samples.push({ durationMs: Number(durationMs.toFixed(3)), responseBytes: bytes, queryCalls: 7,
...
console.log(JSON.stringify({ node: process.version, platform: process.platform, provider: "seed.http", providerVersion: "0.1.0",
  task: "Explicitly select validation, resolve required schema context, read routed knowledge and Specification", acceptance: "Exact expected IDs and zero omissions; configured budgets respected",
